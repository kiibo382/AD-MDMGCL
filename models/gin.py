...
class OGBGNN(nn.Module):
    ...
    def forward(self, g, edge_weight=None):
        ...
        h_node = self.node_gnn(g, x, edge_attr, edge_weight)
...
class GINConv(nn.Module):
    ...
    def forward(self, g, x, edge_attr, edge_weight = None):
        with g.local_scope():
            edge_embedding = self.bond_encoder(edge_attr)
            g.ndata['x'] = x
            g.apply_edges(fn.copy_u('x', 'm'))
            g.edata['m'] = F.relu(g.edata['m'] + edge_embedding)*edge_weight.view(-1, 1) if edge_weight is not None else F.relu(g.edata['m'] + edge_embedding)
...
class GNN_node(nn.Module):
    ...
    def forward(self, g, x, edge_attr, edge_weight = None):
        ...
        for layer in range(self.num_layers):
            ...
            h = self.convs[layer](g, h_list[layer], edge_attr, edge_weight)
