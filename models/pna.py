...
class PNA(nn.Module):
    ...
    def forward(self, graph: dgl.DGLGraph, edge_weight=None, node_weight=None):
        self.node_gnn(graph, edge_weight=edge_weight, node_weight=node_weight)
        readouts_to_cat = [
...
class PNAGNN(nn.Module):
    ...
    def forward(self, graph: dgl.DGLGraph, edge_weight=None, node_weight=None):
        ...
        graph.ndata["feat"] = graph.ndata["feat"] * node_weight.view(-1, 1) if node_weight is not None else graph.ndata["feat"]

        for mp_layer in self.mp_layers:
            mp_layer(graph, edge_weight=edge_weight, node_weight=node_weight)
...
class PNALayer(nn.Module):
    ...
    def forward(self, g, edge_weight=None, node_weight=None):
        h = g.ndata["feat"]
        h_in = h
        # pretransformation
    ...
    def reduce_func(self, nodes) -> Dict[str, torch.Tensor]:
        ...
        if len(self.scalers) > 1:
            h = torch.cat(
                [scale(h, D=D, avg_d=self.avg_d) for scale in self.scalers], dim=-1
            )
    ...
    def pretrans_edges(self, edges) -> Dict[str, torch.Tensor]:
        ...
        """
        edges_src = edges.src["feat"] * edges.data["weight"].view(-1, 1) if "weight" in edges.data else edges.src["feat"]
        edges_dst = edges.dst["feat"] * edges.data["weight"].view(-1, 1) if "weight" in edges.data else edges.dst["feat"]
        if self.edge_features and self.pairwise_distances:
            squared_distance = torch.sum(
                (edges.src["x"] - edges.dst["x"]) ** 2, dim=-1
            )[:, None]
            z2 = torch.cat(
                [
                    edges_src,
            ...
            z2 = torch.cat(
                [edges_src, edges_dst, squared_distance], dim=-1
            )
        elif self.edge_features and not self.pairwise_distances:
            z2 = torch.cat(
                [edges_src, edges_dst, edges.data["feat"]], dim=-1
            )
        else:
            z2 = torch.cat([edges_src, edges_dst], dim=-1)
