...
from commons.utils import flatten_dict, move_to_device, tensorboard_gradient_magnitude
...
class Trainer:
    def __init__(
        ...
        self.main_metric = (
            type(self.loss_func).__name__ if main_metric == "loss" else main_metric
        )
        ...
        if args.checkpoint:
            ...
            self.optim_steps = 0
            self.best_val_score = (
                -np.inf if self.main_metric_goal == "max" else np.inf
            ...
            shutil.copyfile(
                ...
                os.path.join(
                    self.writer.log_dir, os.path.basename(self.args.config.name)
                ),
            )
    ...
    def train(self, train_loader: DataLoader, val_loader: DataLoader):
        ...
        for epoch in range(
            self.start_epoch, self.args.num_epochs + 1
            ...
            with torch.no_grad():
                ...
                if (
                    self.args.eval_per_epochs > 0
                    and epoch % self.args.eval_per_epochs == 0
                ):
                ...
                if self.main_metric == "InfoNCE":
                    ...
                    print(
                        "[Epoch %d] Mutual Information: %.6f"
                        % (epoch, np.log(self.args.batch_size - 1) - val_score)
                    )
                ...
                if (
                    ...
                    and self.main_metric_goal == "max"
                    or val_score <= self.best_val_score
                    and self.main_metric_goal == "min"
                ):
                    epochs_no_improve = 0
                ...
                if (
                    epochs_no_improve >= self.args.patience
                    and epoch >= self.args.minimum_epochs
                ):  # stopping criterion
                    print(
                ...
                if epoch in self.args.models_to_save:
                    shutil.copyfile(
                        os.path.join(self.writer.log_dir, "best_checkpoint.pt"),
                        os.path.join(
                            self.writer.log_dir, f"best_checkpoint_{epoch}epochs.pt"
                        ),
                    )
    ...
    def after_optim_step(self):
        ...
        if self.lr_scheduler != None and (
            ...
            or (
                isinstance(self.lr_scheduler, WarmUpWrapper)
                and self.lr_scheduler.total_warmup_steps > self.lr_scheduler._step
            )
    ...
    def evaluate_metrics(
        self, predictions, targets, batch=None, val=False
    ) -> Dict[str, float]:
        metrics = {}
    ...
    def tensorboard_log(
        self, metrics, data_split: str, epoch: int, step: int, log_hparam: bool = False
    ):
        metrics["epoch"] = epoch
    ...
    def run_tensorboard_functions(self, predictions, targets, step, data_split):
        for key, tensorboard_function in self.tensorboard_functions.items():
            tensorboard_function(
                predictions, targets, self.writer, step, data_split=data_split
            )
    ...
    def initialize_optimizer(self, optim):
        transferred_keys = [
            ...
            if any(transfer_layer in k for transfer_layer in self.args.transfer_layers)
            and not any(
        ...
        frozen_keys = [
            k
            for k in self.model.state_dict().keys()
            if any(to_freeze in k for to_freeze in self.args.frozen_layers)
        ]
        frozen_params = [
            v for k, v in self.model.named_parameters() if k in frozen_keys
        ]
        transferred_params = [
        ...
        new_params = [
            ...
            and "batch_norm" not in k
            and k not in frozen_keys
        ]
        batch_norm_params = [
            v
            for k, v in self.model.named_parameters()
        ...
        transfer_lr = (
            self.args.optimizer_params["lr"]
            if self.args.transferred_lr == None
            else self.args.transferred_lr
        )
