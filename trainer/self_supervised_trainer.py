...
class SelfSupervisedTrainer(Trainer):
    def __init__(self, model, model3d, args, metrics: Dict[str, Callable], main_metric: str,
        ...
        self.node_drop_rate = args.node_drop_rate
        if args.checkpoint:
    ...
    def forward_pass(self, batch):
        ...
        if self.node_drop_rate > 0:
            dist = Bernoulli(self.node_drop_rate)
            dgl_graph_2d = info2d[0].clone()
            dgl_graph_3d = info3d[0].clone()
            for ntype in dgl_graph_2d.ntypes:
                samples = dist.sample(torch.Size([dgl_graph_2d.num_nodes(ntype)]))
                nids_to_remove = dgl_graph_2d.nodes(ntype)[samples.bool().to(dgl_graph_2d.device)]
                dgl_graph_2d.remove_nodes(nids_to_remove, ntype=ntype)
                nids_to_remove = dgl_graph_3d.nodes(ntype)[samples.bool().to(dgl_graph_3d.device)]
                dgl_graph_3d.remove_nodes(nids_to_remove, ntype=ntype)
            view2d = self.model(dgl_graph_2d, *snorm_n)
            view3d = self.model3d(dgl_graph_3d)
